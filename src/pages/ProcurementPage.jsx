...
export default function ProcurementPage() {
  const [tab, setTab]         = useState('egp')
  const [items, setItems]     = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    setLoading(true)
    getProcurement({ type: tab })
    ...
    <div>
      <div className="card">
        <div className="section-head">
          <h2 className="text-sm font-semibold">📦 การจัดซื้อจัดจ้าง</h2>
        </div>
        <div className="flex border-b border-gray-200">
          {[
            { key: 'egp',  label: 'รายงานจัดซื้อจัดจ้าง ภาคงานของรัฐ (EGP)' },
              ...
              {items.map((p, i) => (
                <tr key={p._id} className="border-b border-gray-50 hover:bg-blue-50/50">
                  ...
                  <td className="px-3 py-2.5 text-gray-400 text-xs">
                    {new Date(p.publishedAt).toLocaleDateString('th-TH')}
                  </td>
                </tr>
              ))}
            </tbody>
