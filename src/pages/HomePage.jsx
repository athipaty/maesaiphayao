...
import { normalizeHomeOrder } from '../config/homeSections'
...
// ── Homepage sections ───────────────────────────────────────────────────────
// Each of these renders one reorderable section (see config/homeSections.js).
// Admin picks their order in ตั้งค่าเว็บไซต์ → ลำดับส่วนต่างๆ ในหน้าแรก; HomePage
// below just maps that order onto this registry.

function SectionNews({ allNews, loading }) {
  return (
    <Reveal>
...
function SectionAnnounce({ annItems, annItemWidth, annTrackRef, setLightboxItem, annDesktopItemWidth, annDesktopTrackRef }) {
  return (
    <>
...
function SectionFacebook({ fbContainerRef, fbScale, landingPhoto }) {
  return (
    <Reveal>
...
function SectionNotices({ notices, noticeOpen, setNoticeOpen }) {
...
function SectionProcurement({ egpLoading, egpError, egp, egpOpen, setEgpOpen }) {
  return (
    <Reveal>
...
export default function HomePage() {
    ...
    async function load() {
      try {
        ...
        getSettings().then(r => {
          ...
          setSectionOrder(normalizeHomeOrder(r?.data?.homeSectionOrder))
        }).catch(() => setSectionOrder(normalizeHomeOrder(null)))
  ...
  const sectionProps = {
    ...
    announce:    { annItems, annItemWidth, annTrackRef, setLightboxItem, annDesktopItemWidth, annDesktopTrackRef },
    facebook:    { fbContainerRef, fbScale, landingPhoto },
    notices:     { notices, noticeOpen, setNoticeOpen },
    procurement: { egpLoading, egpError, egp, egpOpen, setEgpOpen },
    travel:      { travel },
    products:    { products },
    videos:      { videos },
  }
  ...
  return (
    <div>
      ...
      {(sectionOrder || normalizeHomeOrder(null)).map(key => {
