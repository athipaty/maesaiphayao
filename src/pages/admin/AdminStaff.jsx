...
function DeptManager({ depts, onChange }) {
  ...
  function removeDept(value) {
    if (
      !window.confirm(
        "ลบกอง/ฝ่ายนี้? บุคลากรที่ผูกกับกองนี้จะยังคงอยู่แต่ชื่อกองจะไม่แสดง",
      )
    )
      return;
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-5">
      ...
      {open && (
        <div className="px-5 pb-5 border-t border-gray-100">
          ...
          {saving && (
            <p className="text-xs text-gray-400 mt-2">กำลังบันทึก...</p>
          )}
        </div>
      )}
    </div>
    ...
    },
    {
      ...
      render: (item) => (
        <span className="text-sm font-medium text-gray-800">
          {item.isVacant ? (
            <em className="text-gray-400">ตำแหน่งว่าง</em>
          ) : (
            item.name
          )}
        </span>
    ...
    },
    {
      ...
      render: (item) => (
        <span className="text-xs text-gray-400">
          {depts.find((d) => d.value === item.department)?.label ||
            item.department}
        </span>
      ),
    },
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                  กอง/ฝ่าย <span className="text-red-400">*</span>
                </label>
                <select
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={data.order}
                    onChange={(e) =>
                      onChange("order", parseInt(e.target.value))
                    }
                    placeholder="1, 2, 3..."
                  />
                </div>
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-gray-200 rounded-lg p-3 hover:border-blue-300 transition-colors flex flex-col items-center">
                  <ImageUpload
                    value={data.image}
              ...
              <div className="flex items-center gap-3 bg-orange-50 border border-orange-100 rounded-lg px-4 py-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-orange-400">
                    จะแสดงเป็น "ตำแหน่งว่าง" โดยไม่ต้องกรอกชื่อ
                  </p>
                </div>
              </div>
