...
export default function AdminStaff() {
  ...
  return (
    <div>
      ...
      {/* ── Search & Filter bar ── */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-4 px-4 py-3 flex flex-col sm:flex-row gap-3">
        {/* Search input */}
        ...
        >
          <option value="all">ทุกกอง/ฝ่าย</option>
        ...
        <div className="flex items-center text-xs text-gray-400 whitespace-nowrap">
          {(search || filterDept !== 'all')
            ? <span>พบ <strong className="text-primary">{filteredItems.length}</strong> / {items.length} รายการ</span>
            : <span>ทั้งหมด <strong className="text-gray-600">{items.length}</strong> รายการ</span>
          }
        </div>
      </div>
