        ...
        <div className={`max-w-[1200px] mx-auto px-3 w-full flex items-center justify-between gap-3 transition-[padding] duration-300 ease-in-out ${scrolled ? 'py-1.5' : 'py-2 lg:py-3'}`}>
          ...
          <Link to="/" className={`flex items-center min-w-0 flex-1 transition-[gap,margin-left] duration-300 ease-in-out ${scrolled ? 'gap-0 ml-0' : 'gap-3 lg:gap-4 lg:ml-10'}`}>
            ...
            <div className="min-w-0">
              <h1 className={`font-bold leading-tight tracking-wide text-white transition-[font-size] duration-300 ease-in-out ${scrolled ? 'text-base lg:text-xl' : 'text-xl lg:text-4xl'}`}>
                องค์การบริหารส่วนตำบลแม่ใส
              </h1>
              <p className={`text-xs lg:text-lg text-white/80 truncate transition-[max-height,opacity] duration-300 ease-in-out overflow-hidden ${scrolled ? 'max-h-0 opacity-0' : 'max-h-6 opacity-100 hidden sm:block'}`}>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <Link to="/admin" target="_blank" rel="noopener noreferrer"
              className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-white/85 backdrop-blur-sm hover:bg-white text-gray-700 border border-gray-200 rounded-lg transition-colors whitespace-nowrap shadow-sm">
              🔐 เข้าสู่ระบบ
            </Link>
